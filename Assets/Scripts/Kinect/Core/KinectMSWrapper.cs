...
public class KinectMSWrapper : MonoBehaviour {
    ...
    /// Struts and constants for interfacing C# with the Kinect.dll 
    [Flags]
    ...
    public static bool PollSkeleton( ref NuiTransformSmoothParameters smoothParameters, ref NuiSkeletonFrame skeletonFrame )
    {
        ...
        int hr = KinectMSWrapper.NuiSkeletonGetNextFrame( 0, ref skeletonFrame );
        if ( hr == 0 )
        {
            newSkeleton = true;
        }
        ...
        if ( newSkeleton )
        {
            hr = KinectMSWrapper.NuiTransformSmooth( ref skeletonFrame, ref smoothParameters );
            if ( hr != 0 )
            {
    ...
    public static bool PollColor( IntPtr colorStreamHandle, ref byte[] videoBuffer, ref Color32[] colorImage )
    {
        ...
        int hr = KinectMSWrapper.NuiImageStreamGetNextFrame( colorStreamHandle, 0, ref imageFramePtr );
        if ( hr == 0 )
        {
            newColor = true;
            ...
            hr = KinectMSWrapper.NuiImageStreamReleaseFrame( colorStreamHandle, imageFramePtr );
        }
#endif
    ...
    public static bool PollDepth( IntPtr depthStreamHandle, bool isNearMode, ref ushort[] depthPlayerData )
    {
        ...
        if ( isNearMode )
        {
            KinectMSWrapper.NuiImageStreamSetImageFrameFlags( depthStreamHandle, NuiImageStreamFlags.EnableNearMode );
        }
        else
        {
            KinectMSWrapper.NuiImageStreamSetImageFrameFlags( depthStreamHandle, NuiImageStreamFlags.None );
        }

        int hr = KinectMSWrapper.NuiImageStreamGetNextFrame( depthStreamHandle, 0, ref imageFramePtr );
        if ( hr == 0 )
        {
            newDepth = true;
            ...
            frameTexture.UnlockRect( 0 );
            hr = KinectMSWrapper.NuiImageStreamReleaseFrame( depthStreamHandle, imageFramePtr );
        }
#endif
